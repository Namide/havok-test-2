...
export class PhysicElement<CurrentPhysicShapeType extends ShapeType, CurrentSize = SizeByShape[CurrentPhysicShapeType]> {
  ...
  constructor({
    ...
    if (shapeType === ShapeType.Box) {
      this.shape = this.world.havok.HP_Shape_CreateBox([0, 0, 0], [0, 0, 0, 1], size as Vector3)[1]
      mass = massRatio * (size as Vector3).reduce((total, side) => total * side)
    } else if (shapeType === ShapeType.Sphere) {
      this.shape = this.world.havok.HP_Shape_CreateSphere([0, 0, 0], size as number)[1]
      ...
      this.shape = this.world.havok.HP_Shape_CreateCapsule(...typedSize)[1]
      mass = massRatio * typedSize[2] * length
    }

    this.world.havok.HP_Body_SetShape(this.body, this.shape);
  ...
  getVelocity() {
    return this.world.havok.HP_Body_GetLinearVelocity(this.body)[1]
  }

  setVelocity(velocity: Vector3) {
    this.world.havok.HP_Body_SetLinearVelocity(this.body, velocity)
  }

  setTransform(translation: Vector3, rotation: Quaternion) {
    this.world.havok.HP_Body_SetQTransform(this.body, [translation, rotation])
  }
  ...
  dispose() {
    ...
    this.world.havok.HP_Shape_Release(this.shape)
  }
}
