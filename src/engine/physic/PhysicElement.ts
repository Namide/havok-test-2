...
export class PhysicElement<CurrentPhysicShapeType extends ShapeType, CurrentSize = SizeByShape[CurrentPhysicShapeType]> {
  ...
  constructor({
    ...
    if (shapeType === ShapeType.Box) {
      this.shape = this.world.physic.havok.HP_Shape_CreateBox([0, 0, 0], [0, 0, 0, 1], size as Vector3)[1]
      mass = massRatio * (size as Vector3).reduce((total, side) => total * side)
    } else if (shapeType === ShapeType.Sphere) {
      this.shape = this.world.physic.havok.HP_Shape_CreateSphere([0, 0, 0], size as number)[1]
      ...
      this.shape = this.world.physic.havok.HP_Shape_CreateCapsule(...typedSize)[1]
    ...
    switch (motionType) {
      ...
      case PhysicMotionType.Dynamic:
        this.world.physic.havok.HP_Body_SetMotionType(this.body, this.world.physic.havok.MotionType.DYNAMIC);
        break
    }
  ...
  setTransform(translation: Vector3, rotation: Quaternion) {
    this.world.physic.havok.HP_Body_SetQTransform(this.body, [translation, rotation])
  }
  ...
  getTransform() {
    const [position, quaternion] = this.world.physic.havok.HP_Body_GetQTransform(this.body)[1];
